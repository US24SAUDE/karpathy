...
export default function TopBar({
  ...
  return (
    <header className="relative z-30 flex h-14 items-center justify-between gap-4 border-b border-ink/5 px-5 glass">
      {/* left: brand */}
      <div className="flex items-center gap-3">
        <motion.div
          ...
          style={{
            background: "linear-gradient(135deg,#B9822D,#6B7E4E)",
            boxShadow: "0 0 20px rgba(185,130,45,0.5)",
          }}
        >
          ✦
        </motion.div>
      ...
      <button
        ...
        className="group hidden items-center gap-3 rounded-xl border border-ink/10 bg-ink/5 px-3 py-1.5 text-xs text-muted transition hover:border-violet/40 hover:bg-ink/10 md:flex"
      >
        <Search size={13} />
        <span className="w-40 text-left">Search & command…</span>
        <kbd className="flex items-center gap-0.5 rounded-md border border-ink/10 bg-ink/5 px-1.5 py-0.5 text-[10px] text-faint">
          <Command size={9} />K
        </kbd>
      </button>
      ...
      <div className="flex items-center gap-4">
        <Vital icon={<Activity size={13} />} color="#6B7E4E">
          <AnimatedNumber value={vitals.requestsPerMin} suffix="/min" />
        </Vital>
        <Vital icon={<Cpu size={13} />} color="#6B7E4E">
